...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto createItem(ItemDto itemDto, Long ownerId) {
        log.info("create item " + ownerId + " " + itemDto.toString());
        ...
        log.info("create item , ownerId={}, itemDto={}, owner={}, requestId={}", ownerId, itemDto, owner.toString(),
                itemDto.getRequestId());
        Item item = itemMapper.toModel(itemDto, null, owner.get(), request);
        Item savedItem = repository.save(item);
        return itemMapper.toDto(savedItem, ownerId, new ArrayList<>(), requestId);
    }
    ...
    public ItemDto updateItem(ItemDto itemDto, Long itemId, Long ownerId) {
        ...
        if (itemDto.getRequestId() != null) {
            ...
            request = Optional.of(new Request());
        }
        ...
        return itemMapper.toDto(savedItem, ownerId, comments, requestId);
    }

    public List<ItemDto> getAll(Long ownerId) {
        List<Item> items = repository.findByOwnerId(ownerId);
        List<ItemDto> itemsDto = items.stream().map(item -> itemMapper.toDto(item, ownerId,
                commentRepository.findAllByItem_id(item.getId()),
                null/*requestRepository.findById(item.getRequest().getId()).get()*/)).toList();
        return itemsDto;
    }
    ...
    public List<ItemDto> search(String text, Long userId) {
        if (text.isEmpty() || text == null) {
            ...
            List<ItemDto> itemsDto = items.stream().map(item -> itemMapper.toDto(item,
                    userId, commentRepository.findAllByItem_id(item.getId()),
                    null/*requestRepository.findById(item.getRequest().getId()).get()*/)).toList();
            return itemsDto;
        }
    }
